    ...
    for (let y = 0; y < nCount; y++) {
      for (let x = 0; x < nCount; x++) {
        if (qrcode.isDark(x, y) && typeTable[x][y] === QRPointType.POS_CENTER) {
          ...
          pointList.push(`<circle fill='none' stroke-width='1' stroke='${posColor}' cx='${
            x + 0.5
          }'
          cy='${y + 0.5}' r='${3}'/>`);
        } else if (
          ...
          if (
            ...
            if (
              ...
              g1.push(
                `<circle cx='${x + 1 + 0.5}' cy='${
                  y + 1 + 0.5
                }' r='${1}' fill='#FFFFFF' stroke='${otherColor}' stroke-width='${rand(
                  0.33,
                  0.6
                )}' />`
              );
              if (qrcode.isDark(x + 1, y + 1)) {
                g1.push(
                  `<circle r='${0.5 * rand(0.5, 1)}' fill='${otherColor}' cx='${
                    x + 1 + 0.5
                  }' cy='${y + 1 + 0.5}'/>`
                );
              }
          ...
          if (x < nCount - 1 && y < nCount - 1) {
            if (
              ...
              g1.push(
                `<circle cx='${x + 1}' cy='${y + 1}' r='${Math.sqrt(
                  1 / 2
                )}' fill='#FFFFFF' stroke='${otherColor}' stroke-width='${rand(
                  0.33,
                  0.6
                )}' />`
              );
