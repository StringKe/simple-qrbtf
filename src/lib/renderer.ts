...
export function defaultViewBox<T extends DefaultRendererOptions>({
  qrcode,
}: T) {
...
export function defaultDrawIcon<T extends DefaultRendererOptions>({
  qrcode,
  icon,
}: T) {
  if (!qrcode) return [];
  ...
  if (icon) {
    ...
    if (icon && iconEnabled) {
      ...
      const transform = `translate(${String(iconXY)},${String(
        iconXY
      )}) scale(${String(iconSize / 100)},${String(iconSize / 100)})`;
      pointList.push(
        `<path d='${sq25}' stroke='#FFF' stroke-width='${
          100 / iconSize
        }' fill='#FFF' transform='${transform}' />`
      );
...
<g>
  <defs>
    ...
    }' d='${sq25}' fill='#FFF' transform='${pathTransform}' />
  </defs>
...
export function builtinDrawIcon<T extends DefaultRendererOptions>({
  qrcode,
  icon,
}: T) {
  if (!qrcode) return [];
...
<g>
  <defs>
    <path id='${
      'defs-path' + randomIdDefs
    }' d='${sq25}' fill='#FFF' transform='${defsTransform}' />
  </defs>
  <clipPath id='${'clip-path' + randomIdClips}'>
    <use xlink:href='${'#defs-path' + randomIdDefs}'  overflow='visible'/>
  </clipPath>
